...
def _insert_rows(table_id: str, rows: List[Dict[str, Any]]) -> int:
    ...
    if errors:
        ...
        print(
            f"{successful} rows succeeded, "
            f"{len(failed_indices)} rows failed for {table_id}"
        )
        return successful
    ...
    try:
        ...
        return (
            json.dumps(
                {
                    "message": (
                        f"OK: inserted {total_inserted} rows from "
                        f"bucket={bucket}, prefix/name={name or prefix}"
                    )
                }
            ),
            200,
